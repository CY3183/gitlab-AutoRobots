...
def ccc(header, infodata, order_status_field_id, flow_recod_field_id,
        date_field_id, depart_field_id, target_entry_id1, max_score_label,
        status_field_id, form_data1, record_id, response_field_id, order_id):
    logging.info("异步任务接收到的target_entry_id1：%s" %target_entry_id1)
    # 如果流程发起成功，则修改表单记录字段
    # 第一次调用 route
    ...
    if data:   # 发起流程成功
        ...
        for id in result_ids:
            ...
            while not response_info:
                ...
                response_info = apis.robot(method, robot_url, robot_header, form_data1, callback=True)  # 调用机器人接口返回的信息
